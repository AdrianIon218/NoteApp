...
function TemporalNotification(props:IProps){
  ...
  function divStyleDuration(duration:number){
    const divStyle = {['--display-time']: duration.toString()+'s'} as React.CSSProperties;
    return divStyle;
  }

  return props.durationSeconds? 
    (<div className={classes.notification} style={divStyleDuration(props.durationSeconds)}> 
      {props.children} 
    </div>) : (<div className={classes.notification}>{props.children}</div>);
}
