...
export default function CategoryContextProvider(props:IProps){
  const categories:{value:string[], setValue:(categories:string[]) => void} = useLocalStorage('categories', []);
  ...
  function deleteCategory(categoryName:string){
    ...
    if(index !== -1){
      categories.value = categories.value.filter((item,indexToDeleted) => indexToDeleted !== index); 
  ...
  return <CategoryContext.Provider value={currentContext}>
  {props.children}
