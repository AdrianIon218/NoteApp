...
export default function NotesContextProvider(props:any){
  const notes:{value:NoteStructure[], setValue:(newNote:NoteStructure[]) => void} = useLocalStorage('notes', []);
  ...
  const modifyNote = (note:NoteStructure)=>{
    const indexNoteToEdit = notes.value.findIndex((item)=> item.id === note.id);
  ...
  return (<NotesContext.Provider value={currentContext}>
    {props.children}
