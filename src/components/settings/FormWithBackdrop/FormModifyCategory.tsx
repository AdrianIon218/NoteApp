...
export default function FormModifyCategory(){
  ...
  const [showNotification, setShowMessage] = useState<JSX.Element|undefined>();
  ...
  function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    if(oldCategoryName != 'none' && categoryCtx.replaceCategory(oldCategoryName,newCategoryName)){
      notesCtx.getNotes().filter((item)=>item.categorySelected === oldCategoryName).forEach((item)=>{
        notesCtx.modifyNote({...item,['categorySelected']:newCategoryName});
      });
      categorySelected.current!.setValue('none');
      categorySelected.current!.updateCategory();
      inputRef.current!.value='';
      setShowMessage(<TemporalNotification hideMessage={()=>setShowMessage(undefined)} durationSeconds={1.5}>Category changed !</TemporalNotification>);
    }
  }

  return (<div className={classes['form-display-flex']}>
  {showNotification}
  ...
  <form onSubmit={submit}>
      ...
      <InputText text='Insert a name' customText="The category must be between 3 and 30 characters" ref={inputRef} minLength={3} maxLength={30}/>
      <button type="submit" className={classes['btn-green']}>Save</button>
    </form>
  </div>);
}