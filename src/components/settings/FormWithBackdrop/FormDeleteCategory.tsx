...
export default function FormDeleteCategory(){
  ...
  function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    if(categoryValues.indexOf(selectedCategory)===-1 && categoryContext.getCategories().includes(selectedCategory)){
      categoryContext.deleteCategory(selectedCategory);
      notesContext.getNotes().filter(note => note.categorySelected === selectedCategory).forEach(note=>{
      ...
      setShowMessage(<TemporalNotification hideMessage={()=>setShowMessage(undefined)} durationSeconds={1.5}>Category deleted !</TemporalNotification>);
    }
  }
  ...
  <div className={classes['form-display-flex']}>
    ...
    <form onSubmit={submit}>
      ...
      <ol>
        ...
        <li>Category "none" will be associated for cards whose category is deleted .</li>
      </ol>
      <button type='submit' className={classes['btn-red']}>Delete category</button>
    </form>
    <ListCategories />
  </div>
  </>);
