...
export default function FormModifyNote({note,closeEditMode}:IProps){
  ...
  function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    if(modifiedNote.title !== note.title || modifiedNote.categorySelected !== note.categorySelected || modifiedNote.text !== note.text){
  ...
  return <div className={classes['form-centered']} ref={panel}>
    {showMessage && (<TemporalNotification hideMessage={hideMessage} durationSeconds={2.2}>Note changed !</TemporalNotification>)}
    <h2>Edit note </h2>
    <form onSubmit={submit}>
      <InputText ref={titleNote} text='Title' customText='The title must be between 3 and 30 characters' minLength={3} maxLength={30} value={note.title} />
      <DropDownBtn labelMessage='Choose a category : ' ref={btnSelectRef} valueDefault={note.categorySelected} />
      <TextArea name='textNote' ref={textNote} value={note.text}/>
      <div className={classes['buttons']}>
        <button>Save</button>
        <button type='button' onClick={resetNote} title='Reset to the last saved content'>Discard changes</button>
      </div>
    </form>
    <button className={classes['close-btn']} onClick={closePanel}>&times;</button>
  </div>
}