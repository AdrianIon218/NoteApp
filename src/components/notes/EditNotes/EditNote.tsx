...
export default function EditNote(){
  ...
  function modifyPanel(note:NoteStructure){
    dispatch({type:StateHiddenPanel.modify, item:note, closeEditMode:closePanel});
  }

  function deletePanel(note:NoteStructure){
    dispatch({type:StateHiddenPanel.delete, item:note, closeEditMode:closePanel});
  }
  ...
  const notesElements = allNotes.map((item, index) => <NoteEditCard key={index} 
          title={item.title} categorySelected={item.categorySelected} text={item.text} 
          modifyItem={()=>{modifyPanel(item)}}
          deleteItem={()=>{deletePanel(item)}} />);

  return (
  <div className={classes['edit-note-container']}>
    {panelContent}
    <h2>
    {allNotes.length === 0 ? 'You have no notes to edit .' : 
      allNotes.length === 1 ? 'You have only 1 note .' :
        'You have ' + allNotes.length.toString() + ' notes that can be modified .'}
    </h2>
    {allNotes.length > 0 && 
