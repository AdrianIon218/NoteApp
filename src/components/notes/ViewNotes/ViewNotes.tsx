...
export default function ViewNote(){
  ...
  const notesElements = allNotes.map((item,index)=>{
    return <NoteItem key={index} title={item.title} text={item.text} categorySelected={item.categorySelected} id={item.id} /> });

  return (<div className={classes['view-note']}>
    <h2>
      { allNotes.length === 0 ? 'You don\'t have any notes' : 
