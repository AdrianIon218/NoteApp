...
export const enum ButtonSelected{
  none , viewNotes , createNote , editNote
}

const sideBarElements = [{ btnValue:ButtonSelected.viewNotes, navTitle:'My Notes'},
...
function NoteNav(props:any){
  ...
  const sideBar = sideBarElements.map((item,index) =>
    <button className={btnSelected === item.btnValue ? classes.selected : classes['not-selected']} 
      name={item.btnValue.toString()} onClick={onChange} key={index}>{item.navTitle}</button>
  );

  return <section className={classes.nav}>
    {sideBar}
  </section>
}
